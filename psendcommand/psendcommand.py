...
ComRun="sh run"
ComCiscoISR=['sh ip route','sh run']
OutCiscoISR=['route','run']
...
def send(data):
	...
	if sMethod == "ssh":
		...
		sTunnel = ('ssh -o ConnectTimeout=250 -o StrictHostKeyChecking=no -l ' + sUID + ' ')
	...
	if sUID=="cisco":	
		...
		i=0
		q = child.expect (['#',pexpect.TIMEOUT,pexpect.EOF])
		while i<len(ComCiscoISR):
			print(ComCiscoISR[i])
			if q==0:
				print('child.sendline '+ ComCiscoISR[i]) 
				child.sendline (ComCiscoISR[i])
			elif q==1:
				print ('return 2')
				return
			elif q==2:
				...
				return
			q = child.expect (['#',pexpect.TIMEOUT,pexpect.EOF])
			result=open(sHostname +'-' + OutCiscoISR[i] + '.txt','wb')
